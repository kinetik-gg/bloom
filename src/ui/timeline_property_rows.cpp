...
namespace bloom::ui {
std::vector<TimelineLayerEntry> timelinePropertyEntries(
    ...
    for (auto layer : layers) {
        ...
        if (const auto source = session.directSourceNodeForLayer(layer.layerId)) {
            ...
            bool heading = false;
            for (const auto& binding : node->parameters) {
                if (session.keyframeDiamondStateForParameter(binding.parameterId) == KeyframeDiamondState::Unsupported) continue;
                if (!heading) { group(QObject::tr("Source")); heading = true; }
                auto name = QString::fromStdString(binding.role);
                if (binding.role == document::kSolidColorParameterRole || binding.role == document::kTextColorParameterRole) name = QObject::tr("Color");
                if (binding.role == document::kTextSizeParameterRole) name = QObject::tr("Size");
...
TimelinePropertyRow::TimelinePropertyRow(CompositionSession& session, QWidget* parent)
    : QWidget(parent), session_(session), label_(new QLabel(this)),
      diamond_(new KeyframeDiamond(session, "", this)), blending_(new kit::KDropdown(this)), color_(new kit::KColorChip(this)) {
    ...
    for (auto*& field : fields_) {
        ...
        connect(field, &kit::KValueField::valueChanged, this, [this] { if (!binding_) commitValues(); });
    }
    blending_->setObjectName("timelinePropertyBlending");
    for (auto mode : core::kBlendModes) blending_->addItem(blendModeDisplayName(mode), QVariant::fromValue(core::blendModeStoredValue(mode)));
    ...
    connect(blending_, &kit::KDropdown::currentIndexChanged, this, [this](int index) {
        if (binding_ || index < 0) return;
        const auto mode = core::blendModeFromStoredValue(blending_->itemData(index).value<std::int64_t>());
        const auto layer = entry_.layerId;
        session_.selectLayer(layer);
        if (mode) (void)session_.setSelectedBlendMode(*mode);
    });
    connect(color_, &kit::KColorChip::colorChanged, this, [this](const kit::KColor& color) {
        if (binding_) return;
        const auto layer = entry_.layerId;
        session_.selectLayer(layer);
        (void)session_.setSelectedTextColor({static_cast<double>(color.red), static_cast<double>(color.green), static_cast<double>(color.blue), static_cast<double>(color.alpha)});
    });
    connect(&session_, &CompositionSession::currentTimeChanged, this, [this] { if (isVisible()) bind(entry_); });
}

void TimelinePropertyRow::bind(const TimelineLayerEntry& entry) {
    binding_ = true;
    entry_ = entry;
    setProperty("parameterId", QVariant::fromValue(static_cast<qulonglong>(entry.parameterId.value())));
    ...
    for (auto* field : fields_) field->hide();
    blending_->hide(); color_->hide(); diamond_->hide();
    if (!group) {
        ...
        int count = vector ? 2 : scalar ? 1 : color && role == document::kSolidColorParameterRole ? 4 : 0;
        std::array<double, 4> values{};
        if (vector) { values[0] = vector->x * (scale ? 100 : 1); values[1] = vector->y * (scale ? 100 : 1); }
        if (scalar) values[0] = *scalar * (opacity ? 100 : 1);
        if (color) values = {color->red, color->green, color->blue, color->alpha};
        for (int i = 0; i < count; ++i) {
            auto* field = fields_[static_cast<std::size_t>(i)];
            field->setRange(opacity ? 0 : size ? 1 : scale || rotation ? -100'000 : -1'000'000,
                opacity ? 100 : size ? document::kMaximumTextSizePixels : scale || rotation ? 100'000 : 1'000'000);
            field->setDecimals(color ? 3 : opacity || size ? 1 : 2);
            field->setSingleStep(color ? 0.01 : 1);
            field->setLabel(vector ? (i == 0 ? "X" : "Y") : color ? QString("RGBA").mid(i, 1) : QString());
            field->setUnit(scale || opacity ? "%" : rotation ? QString::fromUtf8("°") : vector || size ? "px" : "");
        ...
        if (role == document::kBlendModeParameterRole) {
            ...
            if (mode) blending_->setCurrentIndex(static_cast<int>(std::ranges::find(core::kBlendModes, *mode) - core::kBlendModes.begin()));
            blending_->show();
        }
        if (color && role == document::kTextColorParameterRole) {
            color_->setColor({static_cast<float>(color->red), static_cast<float>(color->green), static_cast<float>(color->blue), static_cast<float>(color->alpha)});
            color_->setToolTip(exactColorText(*color)); color_->show();
        }
...
void TimelinePropertyRow::commitValues() {
    ...
    if (role == document::kPositionParameterRole) (void)session_.setSelectedPosition(x, y);
    else if (role == document::kAnchorParameterRole) (void)session_.setSelectedAnchor(x, y);
    else if (role == document::kScaleParameterRole) (void)session_.setSelectedScale(x / 100, y / 100);
    else if (role == document::kRotationParameterRole) (void)session_.setSelectedRotation(x);
    else if (role == document::kOpacityParameterRole) (void)session_.setSelectedOpacity(x / 100);
    else if (role == document::kSolidColorParameterRole) (void)session_.setSelectedSolidColor(color);
    else if (role == document::kTextSizeParameterRole) (void)session_.setSelectedTextSize(x);
}
}
