...
// back on anything that is not a positive finite value. QVariant::toDouble() parses "inf"/"nan"
// successfully and neither `+inf <= 0.0` nor `NaN <= 0.0` is true, so the finiteness test is what
// actually rejects them; a non-finite grid pitch would otherwise reach node layout and painting.
