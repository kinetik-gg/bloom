...
void TimelineWorkAreaStrip::setRuler(TimelineRuler& ruler) {
    ...
    connect(&ruler, &TimelineRuler::axisChanged, this, [this] { preview_.reset(); update(); });
    connect(&session_, &CompositionSession::snapshotChanged, this, [this] { preview_.reset(); update(); });
    connect(&session_, &CompositionSession::compositionChanged, this, [this] { preview_.reset(); update(); });
}
std::optional<TimelineAxis> TimelineWorkAreaStrip::axis() const {
    const auto* composition = session_.composition();
    if (!composition) return std::nullopt;
...
void TimelineWorkAreaStrip::paintEvent(QPaintEvent*) {
    ...
    painter.fillRect(QRectF(left, 0, right - left, height()), kit::withOpacity(kit::color(kit::Color::Accent), kit::kDisabledOpacity));
...
void TimelineWorkAreaStrip::mousePressEvent(QMouseEvent* event) {
    ...
    if (std::min(left, right) > kit::px(kit::Spacing::M)) return;
...
void TimelineWorkAreaStrip::mouseMoveEvent(QMouseEvent* event) {
    ...
    const auto mapping = axis();
    if (!mapping) return;
    auto time = frameTimeForIndex(mapping->frameRate, mapping->duration, mapping->frameIndexForPixel(static_cast<int>(event->position().x())));
    if (mapping->secondsForPixel(event->position().x()) >= mapping->duration.toSeconds()) time = mapping->duration;
    if (!time) return;
    auto next = *preview_;
    if (startHandle_) next.start = *time; else next.end = *time;
    if (next.start < next.end) preview_ = next;
    update();
    event->accept();
}
void TimelineWorkAreaStrip::mouseReleaseEvent(QMouseEvent* event) {
    if (!preview_ || event->button() != Qt::LeftButton) return;
...
void TimelineWorkAreaStrip::keyPressEvent(QKeyEvent* event) {
    if (event->key() == Qt::Key_Escape) { preview_.reset(); update(); event->accept(); return; }
