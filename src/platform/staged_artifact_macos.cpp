...
#error "staged_artifact_macos.cpp requires macOS"
#endif
...
struct MacSharedState final {
    explicit MacSharedState(const StagedArtifactConfig& value) noexcept : config(value) {}
...
class AdmissionToken final {
  public:
    ...
    explicit AdmissionToken(std::shared_ptr<MacSharedState> state) noexcept
  ...
  private:
    ...
    std::shared_ptr<MacSharedState> state_;
};

struct TargetData final {
    std::shared_ptr<MacSharedState> shared;
...
[[nodiscard]] ArtifactTargetFingerprint fingerprintFrom(const struct stat& status,
    ...
    return {.identity = {.first = static_cast<std::uint64_t>(status.st_dev),
            ...
            .byteSize = byteSize,
            .modificationSeconds = static_cast<std::int64_t>(status.st_mtimespec.tv_sec),
            .modificationNanoseconds = static_cast<std::uint32_t>(status.st_mtimespec.tv_nsec),
            .digest = digest};
}
...
[[nodiscard]] bool sameFingerprintEvidence(const struct stat& before,
    ...
    return identityFromStat(before) == identityFromStat(after) && before.st_size == after.st_size &&
           ...
           before.st_mtimespec.tv_nsec == after.st_mtimespec.tv_nsec && isRegular(after);
}
...
[[nodiscard]] std::optional<std::string> makeStageName() {
    ...
    if (::getentropy(nonce.data(), nonce.size()) != 0) {
        return std::nullopt;
    }
...
class MacTarget final : public StagedArtifactTargetState {
  public:
    explicit MacTarget(TargetData data) noexcept : data_(std::move(data)) {}
    ~MacTarget() override = default;
...
class MacLease final : public StagedArtifactLeaseState {
  public:
    MacLease(TargetData target, FileDescriptor descriptor, std::string stageName,
    ...
    ~MacLease() override { cleanupStage(); }
  ...
  private:
    ...
    [[nodiscard]] StagedArtifactError publishAtomically() noexcept {
        if (!target_.expectedTarget.exists) {
            if (::renameatx_np(target_.parentDescriptor.get(), stageName_.c_str(),
                               target_.parentDescriptor.get(), target_.leaf.c_str(),
                               RENAME_EXCL) == 0) {
...
class MacCoordinator final : public StagedArtifactCoordinatorState {
  public:
    explicit MacCoordinator(std::shared_ptr<MacSharedState> state) noexcept
        : state_(std::move(state)) {}
    ...
    stage(std::unique_ptr<StagedArtifactTargetState> target) noexcept override {
        auto* linuxTarget = dynamic_cast<MacTarget*>(target.get());
        ...
        try {
            ...
            for (std::size_t attempt = 0; attempt < maximumAttempts; ++attempt) {
                ...
                auto lease =
                    std::make_unique<MacLease>(std::move(data), std::move(descriptor),
  ...
  private:
    ...
    preflightChecked(const StagedArtifactPreflightRequest& request) {
        ...
        try {
            ...
            return PlatformPreflightResult(std::make_unique<MacTarget>(std::move(data)));
        } catch (...) {
...
createPlatformStagedArtifactCoordinator(const StagedArtifactConfig& config) noexcept {
    try {
        auto shared = std::make_shared<MacSharedState>(config);
        return PlatformCoordinatorResult(std::make_unique<MacCoordinator>(std::move(shared)));
    } catch (const std::bad_alloc&) {
