...
class GpuComposite final {
  public:
    ...
    // instance. Wrong thread returns WrongThread. The two pipelines, layouts, descriptor sets,
    // command pool, and fence are built once on the first begin, under one bounded resident-pool
    // slot acquired before any native allocation; an idle instance allocates nothing.
