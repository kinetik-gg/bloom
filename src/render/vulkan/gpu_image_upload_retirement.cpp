...
// Mirrors the accepted GpuPathCoverage lifecycle: one fixed-capacity slot per native-resource-owning
// Impl, acquired before its first native allocation and held until owner-thread release. A
// foreign-thread destruction only orphans the already-owned slot; the owner drain proves fence
// retirement (or device loss) non-blockingly and then frees on the owner thread. The pool is the
// only retained store, so it is fixed and allocation-free and can never leak per object. Admission
// refuses cleanly when full and recovers after the owner drains: there is no permanent fuse.
//
