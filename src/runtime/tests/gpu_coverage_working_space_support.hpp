...
// an authored signed/HDR translucent Solid, an explicit non-identity CST round trip, a Text layer,
// a real signed/HDR EXR ImageSource whose input is explicitly "sRGB - Texture", and a non-Normal
...
// input transform, so the native gate executes and compares genuine GPU pixels against the
// unchanged CPU evaluator at the documented 2e-6 gate.
...
mixedPlan(const CompositionFormat compositionFormat, const AssetRecord& asset,
    ...
    operations.emplace_back(CompiledImageEffect{
        ...
        CstKernel{std::string{kAcesWorkingSpace}, std::string{kAcesAlternateSpace}}, false, false});
    operations.emplace_back(CompiledImageEffect{
        NodeId::fromRaw(idBase + 25), OperationIndex::fromRaw(1),
        CstKernel{std::string{kAcesAlternateSpace}, std::string{kAcesWorkingSpace}}, false, false});
...
// The default requestFor() is the Bloom Neutral space, so the working-space fixture must override
// it.
