...
// Every helper and test below drives real OCIO shader preparation from BLOOM_GPUSHADER_TOOLS_DIR,
// so the whole section is compiled only when the shader tools are packaged. Without them main()
// reports an honest SKIP (exit 77); there is no silent pass.
...
void testEffectStage(Expectations& expectations, GpuDevice& device,
    ...
    if (!fixtures.aces.has_value()) {
        expectations.expect(false, "effect: the ACES config resolves");
        return;
    }
...
void testDisplayStage(Expectations& expectations, GpuDevice& device,
    ...
    if (!fixtures.neutral.has_value()) {
        expectations.expect(false, "display: the Bloom Neutral config resolves");
        return;
    }
    GpuOcioTransformSpec spec;
