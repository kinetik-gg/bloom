...
// -----------------------------------------------------------------------------------------------
// Off-UI display-program preparation
// -----------------------------------------------------------------------------------------------

struct GpuDisplayProgramPreparer::Impl final {
    Impl(color::ResolvedBloomNeutralConfig resolvedConfig, GpuOcioCompileOptions compileOptions)
        : config(std::move(resolvedConfig)), options(std::move(compileOptions)) {}

    color::ResolvedBloomNeutralConfig config;
    GpuOcioCompileOptions options;
    // Blocking, internally synchronized command cache + persistent compiler. Shared with every
    // display-program request so no second compiler or tool-invocation path exists.
    GpuOcioProgramPreparer preparer;
    // Bounded oracle cache keyed on (display, view): building a CPU display processor is not free.
...
GpuDisplayProgramPreparer::GpuDisplayProgramPreparer(color::ResolvedBloomNeutralConfig config,
                                                     GpuOcioCompileOptions options)
    : impl_(std::make_unique<Impl>(std::move(config), std::move(options))) {}

GpuDisplayProgramPreparer::~GpuDisplayProgramPreparer() = default;

GpuDisplayProgramResult GpuDisplayProgramPreparer::prepare(std::string_view display,
    ...
    if (width == 0 || height == 0 || !viewAdjust.valid()) {
        result.error = GpuDisplayProgramError::InvalidRequest;
        result.diagnostic = "the display program request is invalid";
        return result;
    }
    if (cancel && cancel()) {
        result.error = GpuDisplayProgramError::Cancelled;
        result.diagnostic = "the display program preparation was cancelled";
        return result;
    }
    // An empty display/view selects the resolved config's own default pair. This lets the default
    // Neutral request take the same capacity-based general route as any explicit pair.
    const std::string resolvedDisplay =
        display.empty() ? std::string(impl_->config.displayName()) : std::string(display);
    const std::string resolvedView =
        view.empty() ? std::string(impl_->config.viewName()) : std::string(view);
    if (resolvedDisplay.empty() || resolvedView.empty()) {
        result.error = GpuDisplayProgramError::InvalidRequest;
        result.diagnostic = "the resolved config exposes no default display/view pair";
        return result;
    }
    // Exactness: the producer's DisplayPacking wrapper bakes the exact post-display exposure/gamma
    // (runtime::ViewAdjust::fromEncoded semantics) as constants and binds it into the command
    // identity, so a changed adjustment is a different display command and never reuses an older
    // artifact. Gamma is never mapped to OCIO ExposureContrast. The adjustment rides on the transform
    // spec; the preparer warm key folds both fields.
    const auto& geometry = GpuOcioCommandGeometry{width, height};
    GpuOcioTransformSpec spec;
    spec.kind = GpuOcioTransformKind::Display;
    spec.display = resolvedDisplay;
    spec.view = resolvedView;
    spec.viewAdjust = viewAdjust;

    const auto prepared =
        impl_->preparer.prepare(impl_->config, spec, geometry, impl_->options, cancel);
    ...
    {
        const std::pair<std::string, std::string> key{spec.display, spec.view};
        std::lock_guard lock(impl_->oracleMutex);
        const auto found = impl_->oracleCache.find(key);
        if (found != impl_->oracleCache.end()) {
            oracle = found->second;
        } else {
            auto built = color::buildCpuDisplayProcessorForView(impl_->config, spec.display,
                                                                spec.view);
            ...
            auto taken = std::move(built).takeHandle();
            if (!taken.has_value()) {
                result.error = GpuDisplayProgramError::OracleUnavailable;
                result.diagnostic = "the CPU display oracle could not be taken";
                return result;
            }
            oracle = std::make_shared<const color::PreparedCpuDisplayProcessorHandle>(
                std::move(*taken));
            impl_->oracleCache.emplace(key, oracle);
        }
    }

    result.program.command = prepared.command;
    result.program.cpuOracle = std::move(oracle);
    // The binding is taken from the ACTUAL resolved config, never from the caller's request. The
    // service additionally stamps the locator kind/value from the request.
    result.program.binding.expectedRevision = impl_->config.expectedRevision();
    result.program.binding.workingColorSpaceId = std::string(impl_->config.processColorSpaceId());
...
GpuOcioPreparerCounters GpuDisplayProgramPreparer::counters() const {
    return impl_->preparer.counters();
}
