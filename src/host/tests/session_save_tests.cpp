...
// over a spliced 1.2 fixture, mirroring save_archive_tests.cpp) + schemaMinor 2 at creation ->
// save -> the manifest in the published file declares {1,2} and the unknown member survives
...
void testRoundTrippedNewerMinorGreenChain(Expectations& expectations) {
    ...
    if (manifestDom) {
        ...
        expectations.expect(
            ...
            "round-tripped newer minor: the published manifest declares document schema {1,12}");
    }
...
void testBudgetExhaustionPassThrough(Expectations& expectations) {
    ...
    const CanonicalManifestV1 probeManifest{.documentSchemaVersion = {1, 11},
