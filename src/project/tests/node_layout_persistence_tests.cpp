...
void futureLayoutAttachments() {
    ...
    const auto rootMinor = text.find("\"minor\": 21");
    ...
    text.replace(rootMinor, std::string_view("\"minor\": 21").size(), "\"minor\": 22");
    ...
    const CanonicalManifestV1 manifest{.documentSchemaVersion = {1, 22}};
    ...
    expect(opened.roundTrip && opened.schemaMinor == 22,
