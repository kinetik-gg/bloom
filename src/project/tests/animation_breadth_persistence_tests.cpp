...
void roundTripAndReopen() {
    ...
    expect(text.find("\"minor\": 5") != std::string::npos,
...
std::vector<std::byte> legacyArchive(std::string& documentText) {
    ...
    documentText.replace(minor, std::string_view("\"minor\": 5").size(), "\"minor\": 2");
...
void migrationFromTwelve() {
    ...
    expectedFromLegacy.replace(minor, std::string_view("\"minor\": 2").size(), "\"minor\": 5");
...
void minorGating() {
    ...
    const auto anchor = std::string_view("\"minor\": 5");
    ...
    {
        auto text = baseline;
        text.replace(minor, anchor.size(), "\"minor\": 6");
        auto dom = parseStrictJsonDom(test::toBytes(text), {}, memory());
