...
void roundTripAndReopen() {
    ...
    expect(text.find("\"minor\": 16") != std::string::npos,
...
std::vector<std::byte> legacyArchive(std::string& documentText) {
    ...
    const auto minor = documentText.find("\"minor\": 16");
    if (minor == std::string::npos) {
        throw std::logic_error("legacy minor anchor");
    }
    documentText.replace(minor, std::string_view("\"minor\": 16").size(), "\"minor\": 2");
...
void minorGating() {
    ...
    const auto anchor = std::string_view("\"minor\": 16");
    ...
    {
        auto text = baseline;
        text.replace(minor, anchor.size(), "\"minor\": 17");
        auto dom = parseStrictJsonDom(test::toBytes(text), {}, memory());
