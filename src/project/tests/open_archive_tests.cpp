...
void testOpenMinimalRoundTrip(Expectations& expectations) {
    ...
    expectations.expect(openedValue.schemaMinor == 9, "open minimal: schemaMinor decodes to 9");
    ...
    expectations.expect(openedValue.containerVersion == bloom::document::SchemaVersion{1, 0} &&
                            openedValue.documentSchemaVersion ==
                                bloom::document::SchemaVersion{1, 9},
...
void testOpenComposedRoundTrip(Expectations& expectations) {
    ...
    const CanonicalManifestV1 manifest{.documentSchemaVersion = {1, 9},
...
void testOpenRoundTrippedNewerMinorRoundTrip(Expectations& expectations) {
    ...
    auto openedValue = std::move(opened).takeOpened();
    expectations.expect(openedValue.schemaMinor == 10, "open RT: schemaMinor decodes to 10");
    expectations.expect(openedValue.roundTrip.has_value(),
...
void testDocumentSidePreservedReadOnly(Expectations& expectations) {
    ...
    const std::string minorAnchor = "\"minor\": 9\n  },\n  \"project\"";
    ...
    text.replace(minorPos, std::string_view("\"minor\": 9").size(), "\"minor\": 10");
...
void testContainerCrcFailure(Expectations& expectations) {
    ...
    const auto manifestBytes = manifestBytesOrAbort({1, 9});
    const auto documentBytes = minimalCanonicalDocumentBytesOrAbort();
...
void testVersionDisagreementFailure(Expectations& expectations) {
    ...
    // manifest declares {1,10}; the document is written with root schemaVersion.minor = 9. No
    ...
    expectations.expect(
        ...
        "open version disagreement: manifest {1,10} vs. document root minor 9 fails");
    const auto* failure = opened.failure();
    expectations.expect(
    ...
    if (failure != nullptr) {
        ...
        expectations.expect(
            payload != nullptr &&
                payload->manifestVersion == bloom::document::SchemaVersion{1, 10} &&
                payload->documentVersion == bloom::document::SchemaVersion{1, 9},
