...
void testOpenMinimalRoundTrip(Expectations& expectations) {
    ...
    expectations.expect(openedValue.containerVersion == bloom::document::SchemaVersion{1, 0} &&
                            openedValue.documentSchemaVersion ==
                                bloom::document::SchemaVersion{1, 18},
                        "open minimal: container decodes to {1,0} and the document to {1,18}");
...
void testOpenComposedRoundTrip(Expectations& expectations) {
    ...
    const CanonicalManifestV1 manifest{.documentSchemaVersion = {1, 18},
...
void testOpenRoundTrippedNewerMinorRoundTrip(Expectations& expectations) {
    ...
    const std::string anchor = "\"minor\": 18\n  },\n  \"project\"";
    ...
    text.replace(anchorPos, std::string_view("\"minor\": 18").size(), "\"minor\": 19");
    ...
    const CanonicalManifestV1 manifest{.documentSchemaVersion = {1, 19}, .requirements = {}};
    ...
    auto openedValue = std::move(opened).takeOpened();
    expectations.expect(openedValue.schemaMinor == 19, "open RT: schemaMinor decodes to 19");
    expectations.expect(openedValue.roundTrip.has_value(),
...
void testUnsupportedNodeVersions(Expectations& expectations) {
    ...
    for (const auto& [kind, version] : std::array<std::pair<std::string_view, std::uint32_t>, 9>{
        ...
        for (const std::uint32_t minor : {6U, 14U, 15U}) {
            ...
            const auto schema = text.find("\"minor\": 18");
            text.replace(schema, std::string_view("\"minor\": 18").size(),
...
void testDocumentSidePreservedReadOnly(Expectations& expectations) {
    ...
    const std::string minorAnchor = "\"minor\": 18\n  },\n  \"project\"";
    ...
    text.replace(minorPos, std::string_view("\"minor\": 18").size(), "\"minor\": 19");
    ...
    const auto manifestBytes = manifestBytesOrAbort({1, 19});
...
void testSemanticDecodeFailure(Expectations& expectations) {
    ...
    const auto manifestBytes = manifestBytesOrAbort({1, 18});
    const auto documentBytes = minimalCanonicalDocumentBytesOrAbort();
    std::string corrupted(reinterpret_cast<const char*>(documentBytes.data()),
                          documentBytes.size());
    ...
    if (failure != nullptr) {
        ...
        expectations.expect(
            payload != nullptr &&
                ...
                payload->documentVersion == bloom::document::SchemaVersion{1, 18},
