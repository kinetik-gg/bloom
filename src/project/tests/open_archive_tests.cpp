...
void testOpenMinimalRoundTrip(Expectations& expectations) {
    ...
    expectations.expect(openedValue.schemaMinor == 3, "open minimal: schemaMinor decodes to 3");
    ...
    expectations.expect(openedValue.containerVersion == bloom::document::SchemaVersion{1, 0} &&
                            openedValue.documentSchemaVersion ==
                                bloom::document::SchemaVersion{1, 3},
                        "open minimal: container decodes to {1,0} and the document to {1,3}");
...
void testOpenRoundTrippedNewerMinorRoundTrip(Expectations& expectations) {
    ...
    auto openedValue = std::move(opened).takeOpened();
    expectations.expect(openedValue.schemaMinor == 4, "open RT: schemaMinor decodes to 4");
    expectations.expect(openedValue.roundTrip.has_value(),
...
void testDocumentSidePreservedReadOnly(Expectations& expectations) {
    ...
    const std::string minorAnchor = "\"minor\": 3\n  },\n  \"project\"";
    ...
    text.replace(minorPos, std::string_view("\"minor\": 3").size(), "\"minor\": 4");
...
void testVersionDisagreementFailure(Expectations& expectations) {
    ...
    // manifest declares {1,3}; the document is written with root schemaVersion.minor = 4. No
    ...
    expectations.expect(
        ...
        "open version disagreement: manifest {1,3} vs. document root minor 4 fails");
    const auto* failure = opened.failure();
    expectations.expect(
