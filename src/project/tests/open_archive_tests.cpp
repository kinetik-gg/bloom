...
void testOpenMinimalRoundTrip(Expectations& expectations) {
    ...
    expectations.expect(openedValue.containerVersion == bloom::document::SchemaVersion{1, 0} &&
                            openedValue.documentSchemaVersion ==
                                bloom::document::SchemaVersion{1, 12},
                        "open minimal: container decodes to {1,0} and the document to {1,12}");
...
void testOpenComposedRoundTrip(Expectations& expectations) {
    ...
    const CanonicalManifestV1 manifest{.documentSchemaVersion = {1, 12},
...
void testOpenRoundTrippedNewerMinorRoundTrip(Expectations& expectations) {
    ...
    auto openedValue = std::move(opened).takeOpened();
    expectations.expect(openedValue.schemaMinor == 13, "open RT: schemaMinor decodes to 13");
    expectations.expect(openedValue.roundTrip.has_value(),
...
void testUnsupportedNodeVersions(Expectations& expectations) {
    ...
    for (const auto& [kind, version] : std::array<std::pair<std::string_view, std::uint32_t>, 9>{
        ...
        for (const std::uint32_t minor : {6U, 12U, 13U}) {
            auto text = current;
            ...
            const auto schema = text.find("\"minor\": 12");
            text.replace(schema, std::string_view("\"minor\": 12").size(),
    ...
    for (std::uint32_t minor = 0; minor < 12; ++minor) {
        auto text = current;
        const auto schema = text.find("\"minor\": 12");
        text.replace(schema, std::string_view("\"minor\": 12").size(),
                     "\"minor\": " + std::to_string(minor));
...
void testDocumentSidePreservedReadOnly(Expectations& expectations) {
    ...
    const std::string minorAnchor = "\"minor\": 12\n  },\n  \"project\"";
    ...
    text.replace(minorPos, std::string_view("\"minor\": 12").size(), "\"minor\": 13");
    ...
    const auto manifestBytes = manifestBytesOrAbort({1, 13});
...
void testContainerCrcFailure(Expectations& expectations) {
    ...
    const auto manifestBytes = manifestBytesOrAbort({1, 12});
    const auto documentBytes = minimalCanonicalDocumentBytesOrAbort();
...
void testVersionDisagreementFailure(Expectations& expectations) {
    ...
    // manifest declares {1,12}; the document is written with root schemaVersion.minor = 12. No
    ...
    const CanonicalDocumentV1 documentInput{
        .snapshot = &snapshot, .colorSettings = &colorSettings, .schemaMinor = 13};
    ...
    expectations.expect(
        ...
        "open version disagreement: manifest {1,12} vs. document root minor 13 fails");
    const auto* failure = opened.failure();
    expectations.expect(
    ...
    if (failure != nullptr) {
        ...
        expectations.expect(
            payload != nullptr &&
                payload->manifestVersion == bloom::document::SchemaVersion{1, 12} &&
                payload->documentVersion == bloom::document::SchemaVersion{1, 13},
