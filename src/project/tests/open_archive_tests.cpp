...
void testOpenMinimalRoundTrip(Expectations& expectations) {
    ...
    expectations.expect(openedValue.containerVersion == bloom::document::SchemaVersion{1, 0} &&
                            openedValue.documentSchemaVersion ==
                                bloom::document::SchemaVersion{1, 13},
...
void testOpenRoundTrippedNewerMinorRoundTrip(Expectations& expectations) {
    ...
    auto openedValue = std::move(opened).takeOpened();
    expectations.expect(openedValue.schemaMinor == 14, "open RT: schemaMinor decodes to 13");
    expectations.expect(openedValue.roundTrip.has_value(),
...
void testUnsupportedNodeVersions(Expectations& expectations) {
    ...
    for (const auto& [kind, version] : std::array<std::pair<std::string_view, std::uint32_t>, 9>{
        ...
        for (const std::uint32_t minor : {6U, 13U, 14U}) {
            auto text = current;
            ...
            const auto schema = text.find("\"minor\": 13");
            text.replace(schema, std::string_view("\"minor\": 13").size(),
...
void testDocumentSidePreservedReadOnly(Expectations& expectations) {
    ...
    const std::string minorAnchor = "\"minor\": 13\n  },\n  \"project\"";
    ...
    text.replace(minorPos, std::string_view("\"minor\": 13").size(), "\"minor\": 14");
...
void testVersionDisagreementFailure(Expectations& expectations) {
    ...
    const CanonicalDocumentV1 documentInput{
        .snapshot = &snapshot, .colorSettings = &colorSettings, .schemaMinor = 14};
    ...
    if (failure != nullptr) {
        ...
        expectations.expect(
            payload != nullptr &&
                payload->manifestVersion == bloom::document::SchemaVersion{1, 13} &&
                payload->documentVersion == bloom::document::SchemaVersion{1, 14},
