...
void testOpenMinimalRoundTrip(Expectations& expectations) {
    ...
    expectations.expect(openedValue.schemaMinor == 6, "open minimal: schemaMinor decodes to 4");
    ...
    expectations.expect(openedValue.containerVersion == bloom::document::SchemaVersion{1, 0} &&
                            openedValue.documentSchemaVersion ==
                                bloom::document::SchemaVersion{1, 6},
...
void testOpenRoundTrippedNewerMinorRoundTrip(Expectations& expectations) {
    ...
    const std::string anchor = "\"minor\": 6\n  },\n  \"project\"";
    ...
    text.replace(anchorPos, std::string_view("\"minor\": 6").size(), "\"minor\": 7");
...
void testDocumentSidePreservedReadOnly(Expectations& expectations) {
    ...
    const std::string minorAnchor = "\"minor\": 6\n  },\n  \"project\"";
    ...
    text.replace(minorPos, std::string_view("\"minor\": 6").size(), "\"minor\": 7");
...
void testContainerCrcFailure(Expectations& expectations) {
    ...
    const auto manifestBytes = manifestBytesOrAbort({1, 6});
    const auto documentBytes = minimalCanonicalDocumentBytesOrAbort();
...
void testVersionDisagreementFailure(Expectations& expectations) {
    ...
    if (failure != nullptr) {
        ...
        expectations.expect(
            payload != nullptr &&
                payload->manifestVersion == bloom::document::SchemaVersion{1, 6} &&
                payload->documentVersion == bloom::document::SchemaVersion{1, 7},
