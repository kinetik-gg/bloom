...
void testOpenMinimalRoundTrip(Expectations& expectations) {
    ...
    expectations.expect(openedValue.schemaMinor == 5, "open minimal: schemaMinor decodes to 4");
    ...
    expectations.expect(openedValue.containerVersion == bloom::document::SchemaVersion{1, 0} &&
                            openedValue.documentSchemaVersion ==
                                bloom::document::SchemaVersion{1, 5},
...
void testOpenRoundTrippedNewerMinorRoundTrip(Expectations& expectations) {
    ...
    const std::string anchor = "\"minor\": 5\n  },\n  \"project\"";
    ...
    text.replace(anchorPos, std::string_view("\"minor\": 5").size(), "\"minor\": 6");
...
void testDocumentSidePreservedReadOnly(Expectations& expectations) {
    ...
    const std::string minorAnchor = "\"minor\": 5\n  },\n  \"project\"";
    ...
    text.replace(minorPos, std::string_view("\"minor\": 5").size(), "\"minor\": 6");
...
void testContainerCrcFailure(Expectations& expectations) {
    ...
    const auto manifestBytes = manifestBytesOrAbort({1, 5});
    const auto documentBytes = minimalCanonicalDocumentBytesOrAbort();
...
void testVersionDisagreementFailure(Expectations& expectations) {
    ...
    if (failure != nullptr) {
        ...
        expectations.expect(
            payload != nullptr &&
                payload->manifestVersion == bloom::document::SchemaVersion{1, 5} &&
                payload->documentVersion == bloom::document::SchemaVersion{1, 6},
