...
[[nodiscard]] bool decodeComposition(const JsonValue& node, DecodeState& state,
    ...
    if (const auto* area = node.findMember("workArea"); area && state.documentMinor >= 5) {
        ...
        if (!matchOrderedMembers(*area, rangeKeys, true, state, areaPath, range))
            return false;
        document::WorkArea value;
        {
            const AttachmentScope startScope(state, "start");
            if (!detail::decodeRationalTimeValue(*range[0], state, joinPath(areaPath, "start"),
                                                 value.start))
                return false;
        }
        {
            const AttachmentScope endScope(state, "end");
            if (!detail::decodeRationalTimeValue(*range[1], state, joinPath(areaPath, "end"),
                                                 value.end))
                return false;
        }
        if (value.start < core::RationalTime{} || value.start >= value.end ||
            value.end > out.duration) {
            state.fail(DocumentDecodeError::DomainViolation, areaPath);
            return false;
        }
        out.workArea = value;
    }
