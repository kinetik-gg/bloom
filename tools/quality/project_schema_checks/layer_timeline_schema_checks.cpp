...
void version(const json::Value& schema) {
    ...
    requireExact(
        schema.at("$defs").at("fixedVersion-1.5"),
        R"({"type":"object","required":["major","minor"],"properties":{"major":{"const":1},"minor":{"const":5}},"unevaluatedProperties":false})",
        "version 1.5");
...
void validateDocumentSchemaV1_5(const json::Value& schema) {
    using namespace schema_detail;
    version(schema);
    requireExactString(schema.at("$id"), "urn:kinetik:bloom:schema:project-document:1.5",
                       "document id");
    ...
    requireExact(schema.at("properties").at("schemaVersion"),
                 R"({"$ref":"#/$defs/fixedVersion-1.5"})", "document version");
    ...
    requireExact(
        layer.at("labelColor"),
        R"({"type":"array","minItems":3,"maxItems":3,"items":{"type":"integer","minimum":0,"maximum":255}})",
        "RGB label");
    erase(layer, "labelColor");
    auto& composition = definitions.at("composition-1.0").at("properties");
    requireExact(
        composition.at("workArea"),
        R"({"type":"object","required":["start","end"],"properties":{"start":{"$ref":"#/$defs/rationalTime"},"end":{"$ref":"#/$defs/rationalTime"}},"unevaluatedProperties":false})",
        "work area");
    erase(composition, "workArea");
    erase(definitions, "fixedVersion-1.5");
    previous.at("properties").at("schemaVersion") =
        json::parse(R"({"$ref":"#/$defs/fixedVersion-1.4"})");
...
void validateManifestSchemaV1_5(const json::Value& schema) {
    using namespace schema_detail;
    version(schema);
    requireExactString(schema.at("$id"), "urn:kinetik:bloom:schema:project-manifest:1.5",
                       "manifest id");
    requireExactString(schema.at("title"), "Bloom Project Manifest for Document 1.5",
                       "manifest title");
    auto previous = schema;
    auto& declaration =
    ...
    for (auto& member : previous.at("$defs").asObject())
        if (member.first == "fixedVersion-1.5")
            member.first = "fixedVersion-1.4";
    previous.at("$defs").at("fixedVersion-1.4").at("properties").at("minor") =
        json::parse(R"({"const":4})");
